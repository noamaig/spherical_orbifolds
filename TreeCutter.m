classdef TreeCutter < handle
    %my hacky class for cutting a mesh open
    properties
...
%                 A(A~=0)=rand(nnz(A~=0),1);
               ...
               % [dist,newPath]=graphshortestpath(A,sourceInd, targetInd);
                if isinf(dist)
                    ...
                    obj.visualize();
                    hold on;
                    scatter3(obj.V(sourceInd,1),obj.V(sourceInd,2),obj.V(sourceInd,3),500,'filled');
                    scatter3(obj.V(targetInd,1),obj.V(targetInd,2),obj.V(targetInd,3),500,'filled');
                    error('couldn''t reach target node!');
        ...
        function  splitCenterNode(obj,center ,starPathPairs)
            ...
            for j=1:length(obj.pathPairs)
                ...
                if(isnan(centers(1))~=isnan(centers(2)))
                    figure(5000);
                    obj.visualize();
                    error;
                end
